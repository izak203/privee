...
@include('info.notification')
    ...
    <h1>Bulletin d'elève <a href="{{ route('pdf.moyenne-result') }}" class="btn btn-primary">GENERER PDF</a> </h1>
<div class="row">
    <div class="col-md-6">
    ...
    @forelse($user->moyennes as $moyenne)
      @if($moyenne->type_id === 3 && $moyenne->bulletin_id === 1)
        <tr>
            <!--<td>{{ $moyenne->id }}</td>
    ...
    <td><h2 style="background-color:teal;color:#fff;text-align:center;"> @if((round($total, 2)) >= 10) Bravo {{ Auth::user()->first_name }} - {{ Auth::user()->last_name }}, Vous avez effectué un travai formidable @else Des efforts restent à effectuées @endif</h2></td>  
   </tr>
</table>
@endif
